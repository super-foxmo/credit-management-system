  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="成绩" prop="score">
        <el-input v-model="queryParams.score" placeholder="请输入成绩" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['system:subjectScore:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['system:subjectScore:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['system:subjectScore:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
          v-hasPermi="['system:subjectScore:export']">导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="subjectScoreList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
            v-hasPermi="['system:subjectScore:remove']">删除选课</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { listSubjectScore, getSubjectScore, delSubjectScore, addSubjectScore, updateSubjectScore } from "@/api/system/subjectScore";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSubjectScore(this.queryParams).then(response => {
        this.subjectScoreList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getSubjectScore(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateSubjectScore(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addSubjectScore(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除名称为" ' + subjectName + ' "的选课？').then(function () {
        return delSubjectScore(ids);
      }).then(() => {
        this.getList();
        this.$modal.msgSuccess("删除选课成功");
      }).catch(() => { });
    },
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/subjectScore/export', {
        ...this.queryParams
      }, `subjectScore_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>

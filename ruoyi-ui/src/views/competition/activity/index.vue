  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['competition:activity:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['competition:activity:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['competition:activity:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
          v-hasPermi="['competition:activity:export']">导出</el-button>
      </el-col>
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="加分规则" prop="scoreRule">
          <i class="el-icon-circle-plus-outline" @click="handleRuleAdd"></i>
          &#8195
          <i class="el-icon-remove-outline" @click="handleRuleReduce"></i>
          <div v-for="(item, index) in form.scoreRule" v-bind:key="index">
            <scoreRule clearable :scoreRule="item" :grades="grades" type="date" :ref="`${item.gradeName}-${index}`"
...
<script>
import { listActivity, getActivity, delActivity, addActivity, updateActivity } from "@/api/competition/activity";
import { listEnableGrade } from "@/api/competition/grade";
import scoreRule from "./components/score_rule";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listEnableGrade(this.gradeQueryParams).then(response => {
        this.grades = response.rows
      });
    },
    // 取消按钮
    cancel() {
    ...
    /** 添加加分规则 */
    handleRuleAdd() {
      this.form.scoreRule.push({});
    },
    /** 删除加分规则 */
    handleRuleReduce() {
      this.form.scoreRule.pop();
    },
    /** 搜索按钮操作 */
    handleQuery() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除竞赛信息编号为"' + activityIds + '"的数据项？').then(function () {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('competition/activity/export', {
        ...this.queryParams
